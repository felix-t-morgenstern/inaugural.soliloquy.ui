...
public class ButtonWithStateChangesFromTextLocDisplayTest extends DisplayTest {
    public static void main(String[] args) {
        new DisplayTest().runTest(
                "Button with state changes from text loc display test",
                ...
                () -> DisplayTest.runThenClose("Button with state changes from text loc", 16000),
                ButtonWithStateChangesFromTextLocDisplayTest::populateTopLevelComponent
        );
    }
    ...
    protected static void populateTopLevelComponent(UIModule uiModule,
        ...
        var def = button(0)
                .withRectDefault(
                        rectangle().withColor(Color.RED)
                )
                .withRectHover(
                        rectangle().withColor(Color.GREEN)
                )
                .withRectPressed(
                        rectangle().withColor(Color.BLUE)
                )
                .withTextBlockDef(
                        textBlock(
                                MERRIWEATHER_ID,
